<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:51:"D:\wamp\www\wdata/app/admin\view\data\data_show.php";i:1514876518;s:43:"D:\wamp\www\wdata/app/admin\view\layout.php";i:1510139904;s:49:"D:\wamp\www\wdata/app/admin\view\block\header.php";i:1510139904;s:49:"D:\wamp\www\wdata/app/admin\view\block\footer.php";i:1514879062;}*/ ?>
...
<?php if(($choose_type == 1)): ?>
<h3>使用方法：先点"步骤1"任意入口，弹出success页面，然后再点"步骤2"进入知网，如果弹出非success页面，则切换入口</h3>

<h3 id="link_list"> 步骤1.
    <?php if(is_array($choose_list) || $choose_list instanceof \think\Collection || $choose_list instanceof \think\Paginator): $i = 0; $__LIST__ = $choose_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
        <form id="asder" name="formLogin" method="post" target="_blank" action="<?php echo $vo['loginurl']; ?>">
            <input name="username" type="hidden" value="<?php echo $vo['username']; ?>">
            <input name="password" type="hidden" value="<?php echo $vo['password']; ?>">
            <input type="submit" value="入口：<?php echo $i; ?>">
        </form><br>
    <?php endforeach; endif; else: echo "" ;endif; ?>
</h3>
<h3>步骤2.<a href="<?php echo $webdoor; ?>" target="_blank">点击进入<?php echo $webname; ?></a>
</h3>

<!--类型2：没有点击进入这一项-->
<?php elseif(($choose_type == 2)): ?>
<h3>使用方法：先点任意入口，如果出现“该用户并发已满”，请换个入口；</h3>
<?php if(is_array($choose_list) || $choose_list instanceof \think\Collection || $choose_list instanceof \think\Paginator): $i = 0; $__LIST__ = $choose_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
<form id="form-login" action="<?php echo $vo['loginurl']; ?>" method="post" target="_blank">
    <input type="hidden" value="<?php echo $vo['username']; ?>" name="user">
    <input type="hidden" value="<?php echo $vo['password']; ?>" name="pass">
    <input type="hidden" name="url" value="<?php echo $vo['webdoor']; ?>">
    <input type="submit" value="入口：<?php echo $i; ?>">
</form>
<br>
<br>
...
<?php elseif(($choose_type == 3)): ?>
使用说明:
1.若显示“账号或者密码不正确”更换其他入口既可
<?php if(is_array($choose_list) || $choose_list instanceof \think\Collection || $choose_list instanceof \think\Paginator): $i = 0; $__LIST__ = $choose_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
<form id="form-login" action="<?php echo $vo['loginurl']; ?>" method="post" target="_blank"><input type="submit" value="入口：<?php echo $i; ?>"></form>
<br>
<br>
<?php endforeach; endif; else: echo "" ;endif; ?>
...
<?php elseif(($choose_type == 4)): if(is_array($choose_list) || $choose_list instanceof \think\Collection || $choose_list instanceof \think\Paginator): $i = 0; $__LIST__ = $choose_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
<div id="headDiv">
    普通用户名：<?php echo $vo['username']; ?>  密码：<?php echo $vo['password']; ?>             <a href="<?php echo $vo['loginurl']; ?>" target="_blank"><font color="red">点击登录

        </font></a>
    <?php if(($vo['username'] == '747688447@qq.com')): ?>
...
<h3 id="link_list"> 步骤1.
    <?php if(is_array($choose_list) || $choose_list instanceof \think\Collection || $choose_list instanceof \think\Paginator): $i = 0; $__LIST__ = $choose_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
    <form id="asder" name="formLogin" method="post" target="_blank" action="<?php echo $vo['loginurl']; ?>">
        <input name="user" type="hidden" value="<?php echo $vo['username']; ?>">
        <input name="pass" type="hidden" value="<?php echo $vo['password']; ?>">
        <input type="submit" value="入口：<?php echo $i; ?>">
    </form><br>
    <?php endforeach; endif; else: echo "" ;endif; ?>
</h3>
