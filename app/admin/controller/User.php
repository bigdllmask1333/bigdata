...
class User extends Admin
{
    ...
    public function addUser()
    {
        if ($this->request->isPost()) {
            ...
            $data['expire_time'] = date('Y-m-d H:i:s', strtotime("+$expire_time day", strtotime($i)));
    ...
    public function editUser($id = 0)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $row = UserModel::where('id', $id)->field('role_id,auth,expire_time')->find();
            ...
            /*续费算法START*/
            $expire_times=$row['expire_time'];
            $expire_times1=$data['expire_time'];
            if (strtotime($expire_times) - time() > 0) {
                $i = $expire_times;
            } else {
                $i = date('Y-m-d H:i:s');
            }
            $data['expire_time'] = date('Y-m-d H:i:s', strtotime("+$expire_times1 day", strtotime($i)));
            /*续费算法END*/
        ...
        $row = UserModel::where('id', $id)->field('id,username,role_id,nick,email,mobile,auth,status,expire_time')->find()->toArray();
        if (!$row['auth']) {
...
//            ['title' => '设置权限'],
    ...
    public function info()
    {
        if ($this->request->isPost()) {
            ...
            if(stripos($username,"free")!== false){
                return $this->error('禁止修改测试账号');
            }
            ...
            /*
             * TODO
             * 用户名修改可以在控制台去掉东西改好，而且还可能跟别的用户名重复
            */
//            unset($data['username']); /*用户信息修改不给修改用户名*/
